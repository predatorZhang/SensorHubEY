...
{
    public class CasicPress:ICasicPress
    {
                // Static constants
        private const string TABLE_NAME = "ALARM_PRESS";
        ...
        public int queryCountByDevAndUpTime(string devId, DateTime upTime)
        {
            using (OracleConnection conn = new OracleConnection(OracleHelper.ConnectionStringOrderDistributedTransaction))
            {
                try
                {
                    ...
                    String SQL = "SELECT COUNT(*) FROM ALARM_PRESS WHERE DEVCODE=:DEVCODE AND UPTIME=:UPTIME";
