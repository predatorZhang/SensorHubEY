...
{
    public class CasicTemp:ICasicTemp
    {
                // Static constants
        private const string TABLE_NAME = "ALARM_TEMPERATURE";
        ...
        private const string SQL_INSERT_CASIC_TEMP = "INSERT INTO " + TABLE_NAME
                        + " (DBID,"
                        + " " + COLUMN_DEVID + ","
                        + " " + COLUMN_DDATA + ","
                        + " " + COLUMN_LOGTIME + ","
                        ...
                        + " " + COLUMN_CELL + ")"
                        + " VALUES "
                        + " (SEQ_ALARM_TEMPERATURE_ID.NEXTVAL,"
                        + " " + PARM_DEVID + ","
                        + " " + PARM_DDATA + ","
                        + " " + PARM_LOGTIME + ","
                        + " " + PARM_UPTIME + ","
            ...
            using (OracleConnection conn = new OracleConnection(OracleHelper.ConnectionStringOrderDistributedTransaction))
            {
                try
                {
                    ...
                    foreach (Model.CasicTemp dj in djs)
                    {
                        ...
                        OracleHelper.ExecuteNonQuery(tran, CommandType.Text, SQL_INSERT_CASIC_TEMP, parms);
                    }
        ...
        private static OracleParameter[] GetAdapterParameters()
        {
            ...
            OracleParameter[] parms = OracleHelperParameterCache.GetCachedParameterSet(SQL_INSERT_CASIC_TEMP, "INSERT:UPDATE");
            ...
            if (parms == null)
            {
                ...
                OracleHelperParameterCache.CacheParameterSet(SQL_INSERT_CASIC_TEMP, "INSERT:UPDATE", parms);
            }
        ...
        public int queryCountByDevAndUpTime(string devId, DateTime upTime)
        {
            using (OracleConnection conn = new OracleConnection(OracleHelper.ConnectionStringOrderDistributedTransaction))
            {
                try
                {
                    ...
                    String SQL = "SELECT COUNT(*) FROM ALARM_TEMPERATURE WHERE DEVCODE=:DEVCODE AND UPTIME=:UPTIME";
