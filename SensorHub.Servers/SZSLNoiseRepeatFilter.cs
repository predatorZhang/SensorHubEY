...
{
    public class SZSLNoiseRepeatFilter : TerminatorReceiveFilter
    {
        private SZSLNoiseSwitchFilter switchFilter;
        
        private static Encoding m_Encoding= Encoding.Default;
        private static byte[] m_Terminator = Encoding.Default.GetBytes("T");
        private static BasicRequestInfoParser m_Parser = new BasicRequestInfoParser(":", ",");

        public SZSLNoiseRepeatFilter(SZSLNoiseSwitchFilter switcher)
        ...
        protected override StringRequestInfo ProcessMatchedRequest(byte[] readBuffer, int offset, int length)
        {
          //  var requestInfo = m_Parser.ParseRequestInfo(Encoding.ASCII.GetString(readBuffer, offset, length));
            ...
            return new StringRequestInfo("REPEAT", string.Empty, Encoding.ASCII.GetString(Convert.FromBase64String(String.Empty)).Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries));;
        }
    }
}
